...
vec2 parallaxMap(vec2 texCoord, vec3 viewDirection)
{
    ...
    //get depth after and before collsion for linear interpolation
    //the process get the range in heighmap texture which define the
    //texture coordinate where the layer map bigger than texture map
    float afterDepth = currentDepthMapValue - currentLayderDepth;
    float beforeDepth = texture(depthMap, prevTexCoord).r - currentLayderDepth + layerDepth;
