...
int main(int argc, char *argv[])
{
    ...
    if (!glfwInit()) {
        std::cout << "GLFW initilize Failed" << std::endl;
        return -1;
    }
    ...
    if (!gladLoadGLLoader((GLADloadproc)glfwGetProcAddress)) {
        std::cout << "GLAD initialize error" << std::endl;
        return -1;
    }

    Shader ourShader("shaders/5.2.transform.vs", "shaders/5.2.transform.fs");
    ...
    stbi_set_flip_vertically_on_load(true); //filp the load texture
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //transformation first and rotate
        //fir container
        ...
        transform = glm::mat4(1.0f); //reset it to identiy matrix
        transform = glm::translate(transform, glm::vec3(-0.5f, 0.5f, 0.0f));
